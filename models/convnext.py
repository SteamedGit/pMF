...
def convert_weights_to_jax(jax_params: dict, module_pt, hf: bool = False):
        ...
        if path in pt_params:
            ...
            if any(dc_key in path for dc_key in direct_copy):
                if shape != pt_param.shape:
                    log_for_0(
                        f"[WARNING] shape mismatch for direct copy: expect param '{path}' have shape {shape}, but got {pt_param.shape}"
                    )
                ...
                if shape != pt_param.shape:
                    log_for_0(
                        f"[WARNING] shape mismatch after transpose: expect param '{path}' have shape {shape}, but got {pt_param.shape}"
                    )
                pt_params_flat.append(jnp.asarray(pt_param.detach().numpy()))
            pt_params.pop(path)
        else:
            log_for_0(
                f"[WARNING] missing param '{path}' with shape {shape} from PyTorch model"
            )
    ...
    for path, param in pt_params.items():
        if path in ignore_keys:
            continue
        log_for_0(
            f"[WARNING] params not loaded '{path}' with shape {param.shape} from PyTorch model"
        )
...
def load_convnext_jax_model():
    ...
    # NOTE: Seems unnecessary and may be crashing
